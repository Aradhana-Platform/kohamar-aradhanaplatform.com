...
export default function SingleSongClient({
  song,
  allSongs,
}: {
  song: Song;
  ...
  const upNext =
    currentIndex !== -1
      ? [
          ...allSongs.slice(currentIndex + 1),
          ...allSongs.slice(0, currentIndex),
        ]
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20 pt-10">
      ...
      <div className="max-w-7xl lg:sticky lg:top-20 mx-auto px-4 lg:px-8 mb-4">
        <Link
          href="/songs"
          className="inline-flex items-center gap-2 text-slate-600 hover:text-blue-600 transition-colors bg-white/50 backdrop-blur-sm px-4 py-2 rounded-full border border-slate-200 shadow-sm hover:shadow"
        >
          <svg
              ...
              return (
                ...
                >
                  <div className="relative w-40 aspect-video rounded-lg overflow-hidden bg-slate-200 flex-shrink-0">
                    ...
                    />
                    <span className="absolute bottom-1 right-1 px-1 py-0.5 bg-black/80 text-white text-[10px] rounded font-medium">
                      {nextSong.duration}
                    </span>
                  </div>
